...
const ExclosuredHook = {
  async mounted() {
    ...
    try {
      ...
      // will call into (emit_event, broadcast_event). This must be in
      // place before the WASM module is instantiated, because the module
      // imports these functions at link time.
      window.__exclosured = {
      ...
      // State sync: LiveView -> WASM (event-based)
      ...
      // Handle RPC calls from LiveView (this is how stream_call triggers
      // WASM function execution on the client)
      this.handleEvent("wasm:call", ({ func, args, ref }) => {
        try {
          const fn_ = this.wasmBindgen[func];
